...
export default function PrintDebtSection({ scenario, projectionData }) {
  ...
  const projectedDebtFreeAge = debtFreeRow?.age || Math.max(consumerPayoffAge, mortgagePayoffAge);

  return (
    <div className="print-page" style={{ paddingTop: 24 }}>
      <h2 className="text-xl font-bold text-gray-900 mb-1">Debt Analysis</h2>
      <p className="text-sm text-gray-500 mb-4">Payoff timeline and retirement impact</p>

      <div className="space-y-4">
        {/* KPI Cards */}
        <div className="grid grid-cols-4 gap-3">
          <SummaryCard label="Total Debt" value={formatCurrency(totalDebt)} subtitle="All outstanding" color="danger" />
          <SummaryCard label="Monthly Payments" value={formatCurrency(Math.round(monthlyPayments))} subtitle="Combined debt service" color="sunset" />
          <SummaryCard label="Total Interest" value={formatCurrency(totalInterest)} subtitle="Over repayment period" color="danger" />
          <SummaryCard
            label="Debt-Free By"
          ...
          {scenario.consumerDebt > 0 && (
            <Card>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Starting balance</span><span className="font-medium">{formatCurrency(scenario.consumerDebt)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Interest rate</span><span className="font-medium">{parseFloat((scenario.consumerDebtRate * 100).toFixed(4))}%</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Monthly payment</span><span className="font-medium">{formatCurrency(Math.round((consumerSchedule[1]?.payment || 0) / 12))}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Total interest paid</span><span className="font-medium text-red-600">{formatCurrency(totalConsumerInterest)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Paid off by</span><span className="font-semibold">Age {consumerPayoffAge}</span></div>
                <div className="h-px bg-gray-100 my-1" />
                <div className="flex justify-between"><span className="text-gray-500">Total cost</span><span className="font-bold">{formatCurrency(scenario.consumerDebt + totalConsumerInterest)}</span></div>
              </div>
            </Card>
          )}
          {scenario.mortgageBalance > 0 && (
            <Card>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Starting balance</span><span className="font-medium">{formatCurrency(scenario.mortgageBalance)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Interest rate</span><span className="font-medium">{parseFloat((scenario.mortgageRate * 100).toFixed(4))}%</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Monthly payment</span><span className="font-medium">{formatCurrency(Math.round((mortgageSchedule[1]?.payment || 0) / 12))}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Total interest paid</span><span className="font-medium text-red-600">{formatCurrency(totalMortgageInterest)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Paid off by</span><span className="font-semibold">Age {mortgagePayoffAge}</span></div>
                <div className="h-px bg-gray-100 my-1" />
                <div className="flex justify-between"><span className="text-gray-500">Total cost</span><span className="font-bold">{formatCurrency(scenario.mortgageBalance + totalMortgageInterest)}</span></div>
              </div>
            </Card>
          )}
        </div>
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-center">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-400">
                {projectedDebtFreeAge <= scenario.retirementAge ? 'Debt-free before you retire' : 'Paying debt during retirement'}
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
