...
function buildRows(s) {
  ...
  const rows = [
    ...
    [30+O, 'RRSP Contribution Room','Assumptions_RrspContributionRoom', s.rrspContributionRoom || 0, FMT.currency, false, 'Available RRSP contribution room (depleted by savings, accrued by earned income)'],
    [31+O, 'TFSA Contribution Room','Assumptions_TfsaContributionRoom', s.tfsaContributionRoom || 0, FMT.currency, false, 'Available TFSA contribution room (depleted by deposits, accrues $7,000/yr)'],
    [32+O, 'Other Assets',         'Assumptions_OtherAssets',       other,                           FMT.currency, false, 'Other registered accounts + miscellaneous assets'],

    [34+O, 'Real Estate',          null, null, null],
    [35+O, 'Real Estate Value',    'Assumptions_RealEstateValue',   s.realEstateValue || 0,          FMT.currency, false, 'Current market value of your home'],
    [36+O, 'Is Primary Residence', 'Assumptions_IsPrimaryRes',      b(s.isPrimaryResidence ?? true), FMT.int,  false, '1 = primary residence (exempt from cap gains), 0 = investment'],
    [37+O, 'Est. Cost Basis',      'Assumptions_RealEstateCost',    s.estimatedCostBasis || 0,       FMT.currency, false, 'Purchase price (for cap gains if not primary residence)'],

    [39+O, 'Liabilities',          null, null, null],
    [40+O, 'Mortgage Balance',     'Assumptions_MortgageBalance',   s.mortgageBalance || 0,          FMT.currency, false, 'Remaining mortgage principal'],
    [41+O, 'Mortgage Rate',        'Assumptions_MortgageRate',      s.mortgageRate || 0,             FMT.pct,  false, 'Annual mortgage interest rate'],
    [42+O, 'Mortgage Years Left',  'Assumptions_MortgageYears',     s.mortgageYearsLeft || 0,        FMT.int,  false, 'Years remaining on mortgage'],
    [43+O, 'Consumer Debt',        'Assumptions_ConsumerDebt',      s.consumerDebt || 0,             FMT.currency, false, 'Credit cards, lines of credit, car loans, etc.'],
    [44+O, 'Consumer Debt Rate',   'Assumptions_ConsumerDebtRate',  s.consumerDebtRate || 0,         FMT.pct,  false, 'Average annual interest rate on consumer debt'],
    [45+O, 'Consumer Payoff Age',  'Assumptions_ConsumerPayoffAge', s.consumerDebtPayoffAge || (s.currentAge + 10), FMT.int, false, 'Target age to be consumer debt-free'],
    [46+O, 'Other Debt',           'Assumptions_OtherDebt',         s.otherDebt || 0,                FMT.currency, false, 'Student loans, family loans, other liabilities'],
    [47+O, 'Other Debt Rate',      'Assumptions_OtherDebtRate',     s.otherDebtRate || 0,            FMT.pct,  false, 'Average annual interest rate on other debt'],
    [48+O, 'Other Payoff Age',     'Assumptions_OtherPayoffAge',    s.otherDebtPayoffAge || 70,      FMT.int,  false, 'Target age to be completely debt-free'],

    [50+O, 'Expenses & Returns',   null, null, null],
    [51+O, 'Monthly Savings Target','Assumptions_MonthlySavings',    s.monthlySavings || 0,           FMT.currency, false, 'Target monthly savings during working years (RRSP first, then TFSA, then non-reg)'],
    [52+O, 'Monthly Expenses',     'Assumptions_MonthlyExpenses',   s.monthlyExpenses ?? 4000,       FMT.currency, false, 'Total monthly living expenses (housing, food, transport, etc.)'],
    [53+O, 'Includes Debt Payments','Assumptions_ExpensesIncDebt',  s.expensesIncludeDebt ? 1 : 0,   FMT.int,  false, '1 = expenses already include debt payments (engine subtracts them to avoid double-counting)'],
    [54+O, 'Expense Reduction',    'Assumptions_ExpenseReduction',  s.expenseReductionAtRetirement || 0, FMT.pct, true, 'How much expenses drop in retirement (no commute, paid-off home)'],
    [55+O, 'Inflation Rate',       'Assumptions_Inflation',         s.inflationRate || 0.025,        FMT.pct, true, 'Expected annual price increases (BoC target = 2%)'],
    [56+O, 'Real Return (RRSP)',   'Assumptions_RealReturn',        s.realReturn || 0.04,            FMT.pct, true, 'Investment return above inflation for RRSP accounts'],
    [57+O, 'TFSA Return',          'Assumptions_TfsaReturn',        s.tfsaReturn || (s.realReturn || 0.04), FMT.pct, true, 'Investment return above inflation for TFSA'],
    [58+O, 'Non-Reg Return',       'Assumptions_NonRegReturn',      s.nonRegReturn || (s.realReturn || 0.04), FMT.pct, true, 'Investment return above inflation for non-registered accounts'],

    [60+O, 'Withdrawal Strategy',  null, null, null],
    [61+O, 'Withdrawal Order',     null,                            'TFSA > NonReg > RRSP > Other',  FMT.text, false, 'TFSA first (tax-free), then NonReg, then RRSP (most taxed)'],
    [62+O, 'RRSP Meltdown Enabled','Assumptions_MeltdownEnabled',   b(s.rrspMeltdownEnabled),        FMT.int,  false, '1 = gradually draw down RRSP before 72, 0 = wait for RRIF'],
    [63+O, 'Meltdown Start Age',   'Assumptions_MeltdownStartAge',  s.rrspMeltdownStartAge ?? s.retirementAge, FMT.int, false, 'Age to start RRSP meltdown withdrawals'],
    [64+O, 'Meltdown Target Age',  'Assumptions_MeltdownTargetAge', s.rrspMeltdownTargetAge || 71,   FMT.int,  false, 'Age to finish meltdown (must be \u226471, before RRIF kicks in)'],
    [65+O, 'Meltdown Annual',      'Assumptions_MeltdownAnnual',    s.rrspMeltdownAnnual || 0,       FMT.currency, false, 'Annual meltdown withdrawal amount'],

    [67+O, 'Tax Constants',        null, null, null],
    [68+O, 'Capital Gains Inclusion', 'Assumptions_CapGainsRate',   0.50,                            FMT.pct,  false, 'Fraction of capital gains that are taxable (currently 50%)'],
    [69+O, 'TFSA Annual Limit',    'Assumptions_TfsaAnnualLimit',   7000,                            FMT.currency, false, 'Maximum annual TFSA contribution room'],
    [70+O, 'OAS Clawback Threshold','Assumptions_OasClawbackThresh',93454,                           FMT.currency, false, 'Income above this triggers OAS clawback at 15%'],
    [71+O, 'OAS Clawback Rate',    'Assumptions_OasClawbackRate',   0.15,                            FMT.pct,  false, 'Rate at which OAS is clawed back (15%)'],
    [72+O, 'OAS Max Annual',       'Assumptions_OasMaxAnnual',      8881,                            FMT.currency, false, 'Maximum annual OAS benefit (for clawback cap)'],
  ];

  // Spouse section (couple only) — starts at row 74+O
  if (s.isCouple) {
    ...
    rows.push(
      ...
      [sr++, 'Spouse RRSP Contribution Room', 'Assumptions_SpouseRrspContribRoom', s.spouseRrspContributionRoom || 0, FMT.currency, false, 'Spouse unused RRSP contribution room'],
    );
  }
